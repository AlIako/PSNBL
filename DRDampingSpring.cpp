...
DRDampingSpring::DRDampingSpring(double damping, double spring)
...
void DRDampingSpring::setParameter(double damping, double spring)
{
    //copy parameters
    mDamping = damping;
    mSpring  = spring;
...
void DRDampingSpring::update(double timeSinceLastFrame)
{
    // vector between current and target
    double displace = mCurrent - mTarget;

    // spring acceleration to come to target
    // F = ma = -ksx-kdv // force = -spring*vector - damping*velocity
    double springAccel = (-mSpring * displace) - (mDamping * mVelocity);
